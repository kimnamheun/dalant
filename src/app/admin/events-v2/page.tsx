...
export default function AdminEventsV2Page() {
  ...
  async function updateStatus(id: string, status: string) {
    // Deactivate other active events if activating this one
    if (status === 'active') {
      await supabase
        .from('events')
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-gray-50 to-white">
      <NavHeader title="달란트 잔치 관리 (개선판)" backHref="/admin" />
          ...
          <Card key={event.id} className="border-0 shadow-md">
            <CardContent className="p-4 space-y-3">
              ...
              <div className="flex flex-wrap gap-2">
                <Link href={`/admin/events/${event.id}/products-v2`} className="flex-1 min-w-[120px]">
