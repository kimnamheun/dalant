...
export default function AdminStudentsV2Page() {
  ...
  async function loadData() {
    const [{ data: studentsData }, { data: deptsData }, { data: classesData }] = await Promise.all([
      supabase
        .from('profiles')
        .select('id, name, grade, pin, department_id, class_id, departments(name), classes(name)')
        .eq('role', 'student')
        .order('name'),
    ...
    <div className="min-h-screen bg-gradient-to-b from-gray-50 to-white">
      ...
      <main className="p-4 max-w-3xl mx-auto space-y-4 animate-fade-in">
                  ...
                  <div key={s.id} className="flex items-center gap-3 p-4">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-muted-foreground truncate">
                        {s.departments?.name || '미배정'} · {s.classes?.name || '미배정'} · PIN: {s.pin || '-'}
                      </p>
                    </div>
        ...
        <div className="text-center pt-2">
          <Link href="/admin/students" className="text-xs text-muted-foreground hover:text-foreground">
            ← 기존 학생 관리 페이지로 이동
          </Link>
        </div>
      </main>
