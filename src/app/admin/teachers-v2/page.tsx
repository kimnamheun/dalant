...
export default function AdminTeachersV2Page() {
  ...
  async function loadData() {
    const [{ data: teachersData }, { data: deptsData }, { data: classesData }, { data: studentsData }] = await Promise.all([
      supabase
        .from('profiles')
        .select('id, name, department_id, class_id, departments(name), classes(name)')
        .eq('role', 'teacher')
        .order('name'),
  ...
  async function addTeacher() {
    ...
    // 2. Assign department/class if selected
    ...
    <div className="min-h-screen bg-gradient-to-b from-gray-50 to-white">
      ...
      <main className="p-4 max-w-3xl mx-auto space-y-4 animate-fade-in">
              ...
              <div className="divide-y">
                {filtered.map((t) => (
                  <div key={t.id} className="flex items-center gap-3 p-4">
                    <div className="w-10 h-10 rounded-full gradient-green text-white flex items-center justify-center font-bold">
                      {t.name.charAt(0)}
                    </div>
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-muted-foreground truncate">
                        {t.departments?.name || '부서 미배정'} · {t.classes?.name || '반 미배정'}
                      </p>
                    </div>
        ...
        <div className="text-center pt-2">
          <Link href="/admin/teachers" className="text-xs text-muted-foreground hover:text-foreground">
            ← 기존 교사 관리 페이지로 이동
          </Link>
        </div>
      </main>
